...
public class Ex01GradeBook02 {
    ...
    public static void input() {
        ...
        if(nextIndex<STUDENT_SIZE) {
            student[nextIndex]=new Student();
            message = "학생 번호를 입력해주세요.";
            student[nextIndex].id = ScannerUtil.nextInt(scanner, message);
            message = "학생 이름를 입력해주세요.";
            student[nextIndex].name = ScannerUtil.nextLine(scanner, message);
            message = "학생 국어 점수를 입력해주세요.";
            student[nextIndex].korean = ScannerUtil.nextInt(scanner, message);
            message = "학생 영어 점수를 입력해주세요.";
            student[nextIndex].english = ScannerUtil.nextInt(scanner, message);
            message = "학생 수학 점수를 입력해주세요.";
            student[nextIndex].math = ScannerUtil.nextInt(scanner, message);
            nextIndex++;
        }else {
